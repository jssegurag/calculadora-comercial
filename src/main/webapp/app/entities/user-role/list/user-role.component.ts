...
import { EntityArrayResponseType, UserRoleService } from '../service/user-role.service';
...
export class UserRoleComponent implements OnInit {
  ...
  trackId = (_index: number, item: IUserRole): number => this.userRoleService.getUserRoleIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.userRoles = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IUserRole[] | null): IUserRole[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.userRoleService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
