...
import { sampleWithRequiredData, sampleWithNewData, sampleWithPartialData, sampleWithFullData } from '../position.test-samples';

import { PositionService, RestPosition } from './position.service';

const requireRestSample: RestPosition = {
...
describe('Position Service', () => {
  ...
  describe('Service methods', () => {
    ...
    it('should create a Position', () => {
      const position = { ...sampleWithNewData };
      const returnedFromService = { ...requireRestSample };
      const expected = { ...sampleWithRequiredData };

      service.create(position).subscribe(resp => (expectedResult = resp.body));
    ...
    it('should update a Position', () => {
      const position = { ...sampleWithRequiredData };
      const returnedFromService = { ...requireRestSample };
      const expected = { ...sampleWithRequiredData };

      service.update(position).subscribe(resp => (expectedResult = resp.body));
    ...
    describe('addPositionToCollectionIfMissing', () => {
      it('should add a Position to an empty array', () => {
        const position: IPosition = sampleWithRequiredData;
      ...
      it("should add a Position to an array that doesn't contain it", () => {
        const position: IPosition = sampleWithRequiredData;
        const positionCollection: IPosition[] = [sampleWithPartialData];
        expectedResult = service.addPositionToCollectionIfMissing(positionCollection, position);
        expect(expectedResult).toHaveLength(2);
        expect(expectedResult).toContain(position);
      });
      ...
      it('should add only unique Position to an array', () => {
        const positionArray: IPosition[] = [sampleWithRequiredData, sampleWithPartialData, sampleWithFullData];
        const positionCollection: IPosition[] = [sampleWithRequiredData];
      ...
      it('should accept varargs', () => {
        const position: IPosition = sampleWithRequiredData;
        const position2: IPosition = sampleWithPartialData;
    ...
    describe('comparePosition', () => {
      it('Should return true if both entities are null', () => {
        ...
        const compareResult = service.comparePosition(entity1, entity2);
      ...
      it('Should return false if one entity is null', () => {
        ...
        const compareResult1 = service.comparePosition(entity1, entity2);
        const compareResult2 = service.comparePosition(entity2, entity1);
