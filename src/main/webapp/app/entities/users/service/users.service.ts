...
type RestOf<T extends IUsers | NewUsers> = Omit<T, 'createdDate' | 'lastModifiedDate'> & {
...
export class UsersService {
  ...
  protected convertDateFromClient<T extends IUsers | NewUsers | PartialUpdateUsers>(users: T): RestOf<T> {
    return {
      ...users,
      createdDate: users.createdDate?.toJSON() ?? null,
      lastModifiedDate: users.lastModifiedDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restUsers: RestUsers): IUsers {
    return {
      ...restUsers,
      createdDate: restUsers.createdDate ? dayjs(restUsers.createdDate) : undefined,
      lastModifiedDate: restUsers.lastModifiedDate ? dayjs(restUsers.lastModifiedDate) : undefined,
    };
  }
